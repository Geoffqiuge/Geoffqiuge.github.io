  ...
  function Area(row,colum,doms,areaDirections,curDomIndex){
    ...
    this.move = function(direction){
      ...
      if(!cross){
        ...
        var flag = false;
        flag = this.clearFocus(tmpDomIndex);
        if(dir == 0){
  ...
  function Dom(domId,focusClassName,blurClassName){
    ...
    this.changeStyle = function(focusFlag,_objFocus){
      if(focusFlag){
        ...
        $(this.domTxtId).innerHTML = "<marquee width='" + this.marquee +"'"+ "scrollamount='3'" +">" + this.normalTxt + "</marquee>"
        };
      }else{
...
/*  document.onkeypress = keyPress;
  ...
  */
  ...
  function getDataByAjax(url,successMethed,param){
    ...
    //异步ajax
  function getDataByAjax_t(url,successMethed,param){
    var _in_ajax = window.ActiveXObject ? new ActiveXObject("Microsoft.XMLHTTP") : new XMLHttpRequest();
    if (url != undefined && url != null && url != "") {
      var temp = url.split("?");
       url = temp[0];
    ...
    _in_ajax.open("POST", url, true);
    _in_ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded;charset=utf-8");
    _in_ajax.setRequestHeader("X-Requested-With", "XMLHttpRequest");
    _in_ajax.send(null);
    
    _in_ajax.onreadystatechange = function(){
      if(_in_ajax.readyState==4 && _in_ajax.status == 200){
        if(successMethed!=undefined){
          successMethed(_in_ajax.responseText,param);
        };
      };
  ...
  //异步单线程
  function getDataByAjax_d(url,successMethed,param){
    if (url != undefined && url != null && url != "") {
      var temp = url.split("?"); 
      url = temp[0];
      if (temp.length > 1) { url += "?" + encodeURI(temp[1]); };
  ...
  window.focus_logic["getDataByAjax_t"] = getDataByAjax_t;
  window.focus_logic["getDataByAjax_d"] = getDataByAjax_d;
  ...
  window.focus_logic["getCutedString"] = getCutedString;
  window.focus_logic["getByteLength"] = getByteLength;
